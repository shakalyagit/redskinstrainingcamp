...
class Ajax_pagination{
    ...
    function create_links(){
        ...
        ?>
        <script>
        function getData(page,action='99',per_page,action2='0'){  
            //alert(per_page);
             $('#myLoader').show();
            $.ajax({
                method: "POST",
                url: "<?php echo $this->base_url; ?>"+page,
                data: { page: page,action: action ,per_page: per_page,action2: action2},
                beforeSend: function(){
    ...
    function getAJAXlink($count, $text) {
        ...
        return '<a href="javascript:void(0);"' . $this->anchor_class . ' onclick="'.$this->link_func.'('.$pageCount.','.$this->action.','.$this->per_page.','.$this->action2.')">'. $text .'</a>';
    }
}
